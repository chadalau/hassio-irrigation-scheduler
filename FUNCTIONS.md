...
| `countSchedulesToday` | Quantos horarios HABILITADOS caem no dia da semana de hoje (no `timeZone` do servidor). Alimenta o destaque do bloco `.summary` do card. Diferente de `scheduleStatusToday`, so pergunta "esta na agenda de hoje", nao se ja rodou. |
...
Convencoes de acessibilidade dos controles proprios (nao-HA) do card:

- Todo `<button>` do card leva `type="button"` explicito. O default do HTML e
  `type="submit"`; sem o atributo, um card embutido num `<form>` dispararia submit.
- Os dois toggles (`.toggle`, geral e por horario) usam `role="switch"` +
  `aria-checked`. O `aria-label` e ESTATICO e descreve o que o controle governa
  ("Agendamento automático", "Horário das 06:30") -- o estado ja vem do
  `aria-checked`, entao um nome que muda com o estado ("Agendamento ativo") ou que
  descreve a acao ("Desativar horário") contradiz o que o leitor de tela anuncia.
  O `title` (tooltip para quem usa mouse) leva nome + estado.
- O toggle geral desabilitado (zona sem `switch_entity_id`) tambem recebe nome
  acessivel; sem ele o leitor de tela anunciaria apenas "switch, desligado".
...
| `_renderCard` | Renderiza o cabecalho numa linha so (grade de 5 colunas: icone da zona / nome / chip de estado / toggle geral / engrenagem -- o toggle fica ao lado da engrenagem). Abaixo, o bloco `.summary`: contagem de horarios de hoje (`countSchedulesToday`) + proxima execucao a esquerda, volume medio diario a direita. Depois: barra "Regando" (quando ativa), ultima rega, e duas secoes com titulo (`.section-title`) separadas por filete -- "Reservatorio" (tiles de pH/EC via `_renderReservoirMetrics`, um por reservatorio configurado, mais o bloco `.reservoir-level` com volume/estimativa/refil renderizado UMA vez, ja que a zona rastreia um volume so) e "Agenda automatica" (lista de horarios sempre em ordem crescente via `sortSchedulesByTime`, botao `.add-schedule-button` de largura total e, quando `show_water_now`, o botao rotulado `.water-now-button`). A secao do reservatorio so aparece com pH OU EC configurados **OU** `reservoir_volume_l > 0`. |
| `_renderReservoirMetrics` | Renderiza os tiles de pH e EC de um reservatorio (`.metric`, grade de 2 colunas). Cada tile continua sendo um `button` que abre o dialogo nativo de historico do HA. O rotulo leva o sufixo " R1"/" R2" SO quando os dois reservatorios aparecem; com um so o tile diz apenas "pH"/"EC". O valor do pH vai sem sufixo de unidade (o rotulo do tile ja diz o que e, e a unidade propria do sensor costuma ser "pH", que duplicaria); a EC mantem a unidade real, que e dado e nao rotulo. |
| `_renderScheduleRow` | Renderiza um horario em duas linhas dentro de uma caixa com borda arredondada (`.schedule-row`, mesma moldura do card de light_scheduler): hora + indicador fixo de 7 dias (`dayInitials`, cinza quando o dia nao esta marcado) + icone de status (via `scheduleStatusToday`) na linha de cima -- `!` amber (aviso em `schedule_warnings`, tooltip `"Aviso: ${warning}"`), `check-circle` verde ("Rega de hoje concluída") ou `clock-outline` neutro ("Ainda vai regar hoje"); nenhum icone quando o status e ambiguo. Duracao, volume total e ml por vaso na linha de baixo. O toggle (`.toggle`, botao proprio -- ver `_toggleScheduleEnabled`) fica na coluna mais a esquerda da linha (fora do bloco de info), editar/excluir na coluna mais a direita. |
