...
`,bt=["Seg","Ter","Qua","Qui","Sex","Sáb","Dom"],xt=/^(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?$/;function wt(t){const e=xt.exec(t);if(!e)return null;const i=Number(e[1]),r=Number(e[2]),s=e[3]?Number(e[3]):0;return i>23||r>59||s>59?null:{hour:i,minute:r,second:s}}function At(t){const e=wt(t);if(!e)return t;const i=t.slice(0,t.indexOf(":")),r=String(e.minute).padStart(2,"0");return e.second>0?`${i}:${r}:${String(e.second).padStart(2,"0")}`:`${i}:${r}`}function St(t,e){const i=function(t,e){const i=Number.isFinite(t)?t:0;return i<=0?null:i/3600*(Number.isFinite(e)?Math.max(0,e):0)}(t,e);return null===i?null:1e3*i}function Et(t){return Number.isFinite(t)?t>=1e3?(e=t/1e3,Number.isFinite(e)?Math.round(100*e)/100+" L":"0 L"):Math.round(100*t)/100+" ml":"0 ml";var e}function Ct(t){const e=wt(t);if(!e)return t;return`${String(e.hour).padStart(2,"0")}:${String(e.minute).padStart(2,"0")}:${String(e.second).padStart(2,"0")}`}function kt(t){if(!t||"object"!=typeof t)throw new Error("Configuração inválida para o card de irrigação.");const e=t.entity;if("string"!=typeof e||0===e.length||!e.startsWith("sensor."))throw new Error('O card exige um sensor da integração: "sensor.<zona>_next_run".')}class Dt extends at{constructor(){super(...arguments),this._config={type:"custom:irrigation-schedule-card"},this._now=0,this._dialogOpen=!1,this._settingsOpen=!1,this._settingsFlow="",this._settingsPots="",this._settingsReservoir="",this._editingId=null,this._formTime="06:00",this._formDays=[],this._formDurationMin=15,this._formDurationSec=0,this._formError=!1,this._tickerId=null}static{this.styles=$t}static getConfigElement(){return document.createElement("irrigation-schedule-card-editor")}static getStubConfig(){return{show_next_run:gt,show_water_now:mt,compact:ft}}setConfig(t){kt(t),this._config={...t}}getCardSize(){return this._config.compact?2:4}disconnectedCallback(){super.disconnectedCallback(),this._stopTicker()}updated(t){super.updated(t),this._isWatering()?null===this._tickerId&&(this._tickerId=window.setInterval(()=>{this._now=Date.now()},1e3)):null!==this._tickerId&&this._stopTicker()}render(){if(!this.hass)return this._renderConfigError("O card ainda não recebeu o objeto hass do Home Assistant.");try{if(!this._config.entity)return this._renderConfigError("Configure o card com o sensor da zona: sensor.<zona>_next_run.");if(!this._config.entity.startsWith("sensor."))return this._renderConfigError(`"${this._config.entity}" não é um sensor da integração irrigation_scheduler.`);const t=this._sensorEntity;return t?this._renderCard(t):this._renderConfigError(`Entidade "${this._config.entity}" não encontrada.`)}catch(t){return console.error("[irrigation-schedule-card] render failed",t),this._renderConfigError(`Falha ao renderizar o card: ${t instanceof Error?t.message:String(t)}`)}}_renderConfigError(t){return W`
    ...
    `}_renderCard(t){const e=this._config.compact??ft,i=this._config.show_next_run??gt,r=this._config.show_water_now??mt,s=this._locale(),o=function(t){if(!t||t.toLowerCase().startsWith("pt"))return[...bt];const e=new Intl.DateTimeFormat(t,{weekday:"short"});return Array.from({length:7},(t,i)=>{const r=new Date(2e3,0,3+i,12,0,0);return e.format(r).replace(/\.$/,"")})}(s),n=function(t){if(!Array.isArray(t))return[];const e=[];for(const i of t){if(null===i||"object"!=typeof i)continue;const t=i,r="string"==typeof t.time&&null!==wt(t.time)?Ct(t.time):"",s=Array.isArray(t.days)?t.days.filter(t=>"number"==typeof t&&Number.isInteger(t)&&t>=0&&t<=6):[],o="number"==typeof t.duration&&Number.isFinite(t.duration)&&t.duration>0?t.duration:0;!r||0===s.length||o<=0||e.push({id:"string"==typeof t.id?t.id:"",time:r,days:[...new Set(s)].sort((t,e)=>t-e),duration:o,enabled:"boolean"!=typeof t.enabled||t.enabled})}return e}(t.attributes.schedules),a=this._numberAttr(t,"flow_rate_lph")??0,c=this._numberAttr(t,"number_of_pots")??0,d=this._numberAttr(t,"reservoir_volume_l")??0,l=this._switchEid?this.hass?.states[this._switchEid]:void 0,h=this._binarySensorEid?this.hass?.states[this._binarySensorEid]:void 0,u="on"===h?.state,p="on"===l?.state,g=u?"Regando":p?"Agendada":"Desabilitada",m=u?"status-watering":p?"status-scheduled":"status-disabled",f=this._stringAttr(h,"finishes_at"),_=this._stringAttr(h,"started_at"),v=this._now>0?new Date(this._now).toISOString():(new Date).toISOString(),y=f?function(t,e){const i=Date.parse(t),r=Date.parse(e);return Number.isFinite(i)&&Number.isFinite(r)?Math.max(0,Math.floor((i-r)/1e3)):0}(f,v):0,$=_&&f?function(t,e,i){const r=Date.parse(t),s=Date.parse(e),o=Date.parse(i);if(!Number.isFinite(r)||!Number.isFinite(s)||!Number.isFinite(o))return 0;const n=r-s;return n<=0?100:Math.min(100,Math.max(0,(o-s)/n*100))}(f,_,v):0;return W`
      <ha-card class=${e?"compact":""}>
        ...
        <div class="card-body">
          <div class="schedules">
            ${0===n.length?W`<div class="empty">Nenhum horário configurado.</div>`:n.map(t=>this._renderScheduleRow(t,o,s,a,c))}
          </div>
    ...
    `}_renderScheduleRow(t,e,i,r,s){const o=St(r,t.duration),n=function(t,e,i){const r=St(t,e);return null===r?null:r*(Number.isFinite(i)&&i>0?i:1)}(r,t.duration,s);return W`
      <div class="schedule-row">
        ...
        <div class="schedule-duration">
          ${function(t){const e=Math.max(0,Math.round(Number.isFinite(t)?t:0));if(e<60)return`${e} s`;const i=Math.round(e/60),r=Math.floor(i/60),s=i%60,o=[];return r>0&&o.push(`${r} h`),s>0&&o.push(`${s} min`),o.join(" ")}(t.duration)}
          ${null!==n?W`<span class="schedule-volume">≈ ${Et(n)}</span>`:""}
          ${null!==n&&null!==o?W`<span class="schedule-perpot">· ${Et(o)}/vaso</span>`:""}
        </div>
        <ha-switch
          ?checked=${t.enabled}
    ...
    `;var a}_renderSettings(t,e,i){return this._settingsOpen?W`
