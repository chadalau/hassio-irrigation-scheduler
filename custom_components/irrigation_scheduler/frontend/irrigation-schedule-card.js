...
`,bt=["Seg","Ter","Qua","Qui","Sex","Sáb","Dom"],xt=/^(\d{1,2}):(\d{1,2})(?::(\d{1,2}))?$/;function wt(t){const e=xt.exec(t);if(!e)return null;const i=Number(e[1]),r=Number(e[2]),s=e[3]?Number(e[3]):0;return i>23||r>59||s>59?null:{hour:i,minute:r,second:s}}function At(t){const e=wt(t);if(!e)return t;const i=t.slice(0,t.indexOf(":")),r=String(e.minute).padStart(2,"0");return e.second>0?`${i}:${r}:${String(e.second).padStart(2,"0")}`:`${i}:${r}`}function St(t){const e=Math.max(0,Math.round(Number.isFinite(t)?t:0));if(e<60)return`${e} s`;const i=Math.round(e/60),r=Math.floor(i/60),s=i%60,o=[];return r>0&&o.push(`${r} h`),s>0&&o.push(`${s} min`),o.join(" ")}function Et(t,e){const i=Number.isFinite(t)?t:0;if(i<=0)return null;return i/3600*(Number.isFinite(e)?Math.max(0,e):0)}function Ct(t){if(!Number.isFinite(t))return"0 L";return`${Math.round(100*t)/100} L`}function kt(t,e,i){const r=Et(t,e);return null===r||!Number.isFinite(i)||i<=0?null:1e3*r/i}function Dt(t){return Number.isFinite(t)?t>=1e3?Ct(t/1e3):Math.round(100*t)/100+" ml":"0 ml"}function Mt(t){const e=wt(t);if(!e)return t;return`${String(e.hour).padStart(2,"0")}:${String(e.minute).padStart(2,"0")}:${String(e.second).padStart(2,"0")}`}function Nt(t){if(!t||"object"!=typeof t)throw new Error("Configuração inválida para o card de irrigação.");const e=t.entity;if("string"!=typeof e||0===e.length||!e.startsWith("sensor."))throw new Error('O card exige um sensor da integração: "sensor.<zona>_next_run".')}class Pt extends at{constructor(){super(...arguments),this._config={type:"custom:irrigation-schedule-card"},this._now=0,this._dialogOpen=!1,this._settingsOpen=!1,this._settingsFlow="",this._settingsPots="",this._editingId=null,this._formTime="06:00",this._formDays=[],this._formDurationMin=15,this._formDurationSec=0,this._formError=!1,this._tickerId=null}static{this.styles=$t}static getConfigElement(){return document.createElement("irrigation-schedule-card-editor")}static getStubConfig(){return{show_next_run:gt,show_water_now:ft,compact:_t}}setConfig(t){Nt(t),this._config={...t}}getCardSize(){return this._config.compact?2:4}disconnectedCallback(){super.disconnectedCallback(),this._stopTicker()}updated(t){super.updated(t),this._isWatering()?null===this._tickerId&&(this._tickerId=window.setInterval(()=>{this._now=Date.now()},1e3)):null!==this._tickerId&&this._stopTicker()}render(){if(!this.hass)return this._renderConfigError("O card ainda não recebeu o objeto hass do Home Assistant.");try{if(!this._config.entity)return this._renderConfigError("Configure o card com o sensor da zona: sensor.<zona>_next_run.");if(!this._config.entity.startsWith("sensor."))return this._renderConfigError(`"${this._config.entity}" não é um sensor da integração irrigation_scheduler.`);const t=this._sensorEntity;return t?this._renderCard(t):this._renderConfigError(`Entidade "${this._config.entity}" não encontrada.`)}catch(t){return console.error("[irrigation-schedule-card] render failed",t),this._renderConfigError(`Falha ao renderizar o card: ${t instanceof Error?t.message:String(t)}`)}}_renderConfigError(t){return W`
    ...
    `}_renderCard(t){const e=this._config.compact??_t,i=this._config.show_next_run??gt,r=this._config.show_water_now??ft,s=this._locale(),o=function(t){if(!t||t.toLowerCase().startsWith("pt"))return[...bt];const e=new Intl.DateTimeFormat(t,{weekday:"short"});return Array.from({length:7},(t,i)=>{const r=new Date(2e3,0,3+i,12,0,0);return e.format(r).replace(/\.$/,"")})}(s),n=function(t){if(!Array.isArray(t))return[];const e=[];for(const i of t){if(null===i||"object"!=typeof i)continue;const t=i,r="string"==typeof t.time&&null!==wt(t.time)?Mt(t.time):"",s=Array.isArray(t.days)?t.days.filter(t=>"number"==typeof t&&Number.isInteger(t)&&t>=0&&t<=6):[],o="number"==typeof t.duration&&Number.isFinite(t.duration)&&t.duration>0?t.duration:0;!r||0===s.length||o<=0||e.push({id:"string"==typeof t.id?t.id:"",time:r,days:[...new Set(s)].sort((t,e)=>t-e),duration:o,enabled:"boolean"!=typeof t.enabled||t.enabled})}return e}(t.attributes.schedules),a=this._numberAttr(t,"default_duration")??0,c=this._numberAttr(t,"flow_rate_lph")??0,d=this._numberAttr(t,"number_of_pots")??0,l=this._switchEid?this.hass?.states[this._switchEid]:void 0,h=this._binarySensorEid?this.hass?.states[this._binarySensorEid]:void 0,u="on"===h?.state,p="on"===l?.state,g=u?"Regando":p?"Agendada":"Desabilitada",f=u?"status-watering":p?"status-scheduled":"status-disabled",_=this._stringAttr(h,"finishes_at"),m=this._stringAttr(h,"started_at"),v=this._now>0?new Date(this._now).toISOString():(new Date).toISOString(),y=_?function(t,e){const i=Date.parse(t),r=Date.parse(e);return Number.isFinite(i)&&Number.isFinite(r)?Math.max(0,Math.floor((i-r)/1e3)):0}(_,v):0,$=m&&_?function(t,e,i){const r=Date.parse(t),s=Date.parse(e),o=Date.parse(i);if(!Number.isFinite(r)||!Number.isFinite(s)||!Number.isFinite(o))return 0;const n=r-s;return n<=0?100:Math.min(100,Math.max(0,(o-s)/n*100))}(_,m,v):0,b=this._waterNowLabel(a,c,d);return W`
      <ha-card class=${e?"compact":""}>
        <div class="header">
          ...
          <div class="header-right">
            <span class="status ${f}">${g}</span>
            ${l?W`
                  <ha-switch
                    .checked=${p}
                    title=${p?"Agendamento ativo":"Agendamento desativado"}
                    @change=${t=>this._toggleMaster(l,t)}
                  ></ha-switch>
    ...
    `}_renderScheduleRow(t,e,i,r,s){const o=Et(r,t.duration),n=kt(r,t.duration,s);return W`
      <div class="schedule-row">
        ...
        <div class="schedule-duration">
          ${St(t.duration)}
          ${null!==o?W`<span class="schedule-volume">≈ ${Ct(o)}</span>`:""}
          ${null!==o&&null!==n?W`<span class="schedule-perpot">· ${Dt(n)}/vaso</span>`:""}
        </div>
        <ha-switch
          ?checked=${t.enabled}
    ...
    `;var a}_waterNowLabel(t,e,i){if(t<=0)return"Regar agora";const r=`Regar agora por ${St(t)}`,s=Et(e,t);if(null===s)return r;const o=kt(e,t,i);return null!==o?`${r} (≈ ${Ct(s)} · ${Dt(o)}/vaso)`:`${r} (≈ ${Ct(s)})`}_renderSettings(t,e){return this._settingsOpen?W`
    ...
    `:W``}_openSettings(){this._settingsOpen=!this._settingsOpen}_closeSettings(){this._settingsOpen=!1,this._settingsFlow="",this._settingsPots=""}_onSettingsFlowChange(t){this._settingsFlow=t.target.value}_onSettingsPotsChange(t){this._settingsPots=t.target.value}_saveSettings(){const t=Number.parseInt(this._settingsFlow,10),e=Number.parseInt(this._settingsPots,10),i={};Number.isFinite(t)&&t>=0&&(i.flow_rate_lph=t),Number.isFinite(e)&&e>=0&&(i.number_of_pots=e),this._callService("set_zone_options",i),this._closeSettings()}_renderDialog(t){return this._dialogOpen?W`
